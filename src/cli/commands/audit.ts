...
export function createAuditCommand(): Command {
  const command = new Command('audit').description('Analyze Linear workspace').action(async () => {
    try {
      ...
      if (result.projectBreakdown.length > 0) {
        ...
        for (const project of result.projectBreakdown) {
          const pct =
            project.issues > 0 ? Math.round((project.completed / project.issues) * 100) : 0;
          const progressBar = buildProgressBar(pct, 20);
          console.log(
            chalk.white(`  ${project.name.padEnd(25)} `) +
              chalk.gray(
                `${String(project.completed).padStart(2)}/${String(project.issues).padStart(2)} `,
              ) +
              progressBar +
              chalk.gray(` ${pct}%`),
          );
