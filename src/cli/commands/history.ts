...
import { ExecutionStore, EXECUTION_STORE } from '../../domain/interfaces/ExecutionStore.js';
...
export function createHistoryCommand(): Command {
  const command = new Command('history')
    ...
    .action(async (options: { limit: string; issue?: string }) => {
      ...
      try {
        ...
        for (const record of records) {
          ...
          const statusColor = STATUS_COLORS[record.status] ?? chalk.white;
          const status = statusColor(record.status.padEnd(colDefs.status.width));
          ...
          const duration = record.duration
            ? `${(record.duration / 1000).toFixed(1)}s`.padEnd(colDefs.duration.width)
            : '—'.padEnd(colDefs.duration.width);

          console.log(`${time} ${issue} ${title} ${status} ${agent} ${duration}`);
        }

        const suffix = options.issue ? `for issue ${options.issue}` : `of ${limit}`;
