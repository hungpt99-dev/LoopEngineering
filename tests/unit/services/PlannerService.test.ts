...
import { Complexity, IssueStatus, Priority } from '../../../src/domain/value-objects/IssueStatus.js';
...
function mockContextBuilder() {
  return {
    ...
    detectDependencies: vi.fn().mockResolvedValue(['lodash', 'express']),
    identifyFiles: vi.fn().mockResolvedValue(['src/index.ts', 'src/app.ts']),
  };
}
...
describe('PlannerServiceImpl', () => {
  ...
  describe('estimateComplexity', () => {
    it('should return LOW for short description', async () => {
      const issue = createIssue({ description: 'Brief desc' });
      const result = await service.estimateComplexity(issue);
      expect(result).toBe(Complexity.LOW);
    });

    it('should return MEDIUM for medium description', async () => {
    ...
    it('should return HIGH for long description with high estimate', async () => {
      const issue = createIssue({
    ...
    it('should return VERY_HIGH with blockers and refactoring keywords', async () => {
      const issue = createIssue({
    ...
    it('should increase complexity with parent issue', async () => {
      const issue = createIssue({
  ...
  describe('planImplementation', () => {
    it('should return default step when description is empty', async () => {
      const issue = createIssue({ title: 'My issue', description: '' });
      const steps = await service.planImplementation(issue);
    ...
    it('should parse bullet point steps', async () => {
      const issue = createIssue({
    ...
    it('should parse mixed numbered and bullet steps', async () => {
      const issue = createIssue({
        title: 'Fix things',
    ...
    it('should split plain text by sentences', async () => {
      const issue = createIssue({
        title: 'Feature',
        description: 'First we need to set up the database. Then we should create the API endpoints. Finally we add the frontend components.',
      });
      const steps = await service.planImplementation(issue);
  ...
  describe('selectAgent', () => {
    const agents: AgentInfo[] = [
      ...
      { name: 'opencode', capabilities: ['full-stack', 'architecture', 'refactoring'], enabled: true, priority: 15 },
      { name: 'claude', capabilities: ['frontend', 'react'], enabled: true, priority: 10 },
    ];

    it('should select highest scoring agent', async () => {
      ...
      const result = await service.selectAgent(issue, plan, agents);
    ...
    it('should throw when no agents are enabled', async () => {
      ...
      await expect(service.selectAgent(issue, plan, disabledAgents)).rejects.toThrow('No enabled agents available');
    });
  });
});
